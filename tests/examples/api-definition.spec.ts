...
describe('api definition example', () => {
    function doThingAsync (name: string, id: number, mode: 'cat1' | 'cat2'): Promise<string> {
        return Promise.resolve(mode === 'cat1' ? `${name}/${id}` : `${name}-${id}`);
    }
    const doThingPipeline = usePipelineExecution<string, [name: string, id: number, mode: 'cat1'|'cat2']>(doThingAsync)
    const api = {
