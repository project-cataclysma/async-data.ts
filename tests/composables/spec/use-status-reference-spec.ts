...
export async function specExtractsResult(
  dataFn: DelayStatusFunctionComposable,
): Promise<void> {
  const data = dataFn();
...
export async function specExtractsStatus(
  dataFn: DelayStatusFunctionComposable,
) {
  const data = dataFn();
...
export async function specExtractsError(dataFn: DelayStatusFunctionComposable) {
  ...
  expect(
    data.execute("errored-hard"),
    "hard error to be raised",
  ).rejects.toThrow("bad bad");
}
